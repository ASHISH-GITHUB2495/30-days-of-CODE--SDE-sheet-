...
// Actually this programm is for finding the merge overlapping intervals
	// here we gonna do with time complexity O(nlogn) + O(n^2);
	...
	for (auto i : v)
	{
		...
		if (flag != 2)
		{	pair<int, int> p; p = make_pair(i.first, i.second);
			for (auto k : v)
			{
				...
				if (k != i)
				{
					...
					if (k.first >= p.first && k.first <= p.second)
					{
						p.second = max(p.second, k.second);
